...
module.exports = async (req, res) => {
  ...
  query = `SELECT HEX(id) id, HEX(teamId) teamId, HEX(formulaId) formulaId, HEX(tryouts.userId) ownerId, name,
    ballParkLocation, ballParkNotes, street, city, state, zip, country, date, tryouts.createdAt createdAt,
    tryouts.updatedAt updatedAt FROM tryouts LEFT JOIN tryoutCoaches on id = tryoutId WHERE tryoutCoaches.userId =
    UNHEX(?) AND tryoutId = UNHEX(?)`;
